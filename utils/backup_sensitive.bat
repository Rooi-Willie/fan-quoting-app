...
if exist "..\deploy\bin\cloud_sql_proxy.exe" (
    copy "..\deploy\bin\cloud_sql_proxy.exe" "%BACKUP_DIR%\" >nul
    echo [✓] deploy\bin\cloud_sql_proxy.exe
) else (
    echo [✗] deploy\bin\cloud_sql_proxy.exe - not found (OK, will download if needed)
)
...
echo cloud_sql_proxy.exe --^> fan-quoting-app\deploy\bin\ >> "%BACKUP_DIR%\RESTORE_INSTRUCTIONS.txt"
echo. >> "%BACKUP_DIR%\RESTORE_INSTRUCTIONS.txt"
echo. >> "%BACKUP_DIR%\RESTORE_INSTRUCTIONS.txt"
...
echo     copy "cloud_sql_proxy.exe" "..\..\fan-quoting-app\deploy\bin\" ^>nul >> "%BACKUP_DIR%\restore_this_backup.bat"
