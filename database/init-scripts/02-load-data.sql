...
COPY labor_rates(rate_name, rate_per_hour, currency) FROM '/csv_data/labor_rates.csv' WITH (FORMAT csv, HEADER true);
...
COPY fan_configurations FROM '/csv_data/fan_configurations.csv' WITH (FORMAT csv, HEADER true);
...
COPY component_parameters(component_id, default_thickness_mm, default_fabrication_waste_factor, diameter_formula_type, length_formula_type, stiffening_formula_type, mass_formula_type, cost_formula_type) FROM '/csv_data/component_parameters.csv' WITH (FORMAT csv, HEADER true, NULL '');
